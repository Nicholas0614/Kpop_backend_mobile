...
public class PaymentController {
    ...
    public ResponseEntity<Object> capture(@PathVariable String id) throws Exception {
        String result = payPalService.capturePayment(id);

        if (result.equals("COMPLETED")) return orderService.paymentSuccess(id);

        return ResponseEntity.badRequest().body("Payment failed: " + result);
    }
}